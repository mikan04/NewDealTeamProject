...
public interface QnaReRepository extends JpaRepository<QnaReEntity, Long> {
	...
	@Query("SELECT COUNT(DISTINCT qnaReplyRef) FROM QnaReEntity WHERE qna_num = :id AND qna_reply_depth = 0")
	public int replycount(long id);
	
	@Query("SELECT sr FROM QnaReEntity sr WHERE qna_num = :qnaNum ORDER BY qnaReplyRef ASC, qna_num ASC")
