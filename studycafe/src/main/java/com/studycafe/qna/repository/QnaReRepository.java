...
public interface QnaReRepository extends JpaRepository<QnaReEntity, Long> {
	...
	@Query("SELECT COUNT(DISTINCT qnaReRef) FROM QnaReEntity WHERE qna_num = :id AND qna_re_depth = 0")
	public int replycount(long id);
	
	@Query("SELECT sr FROM QnaReEntity sr WHERE qna_num = :qnaNum ORDER BY qna_re_ref ASC, qna_num ASC")
